const app = {
    ...
    fetchWeather: (ev) => {
        ...
        let key = 'bc47b6882013870e0ba2a6f5ff3a8fd3';
        let lang = 'en';
        let units = 'metric';
    ...
    showWeather: (resp) => {
        ...
        // row.innerHTML = '';
        row.innerHTML = resp.daily // daily response in loop
            .map((day, idx) => {
                if (idx <= 7) {//7days week
                    let dt = new Date(day.dt * 1000); //timestamp * 1000 due to JS timing
                    let sr = new Date(day.sunrise * 1000).toTimeString();
                    let ss = new Date(day.sunset * 1000).toTimeString();
                    return `<div class="col">
              <div class="card">
              <h5 class="card-title p-7">${dt.toDateString()}</h5>
                <img
